...
bool Image::saveToFile(const std::string& path) const {
	...
	if (extension == ".png") {
		return stbi_write_png(path.c_str(), static_cast<int>(size_.x), static_cast<int>(size_.y), 4, pixels_.data(),
							  stride) != 0;
	}
	if (extension == ".bmp") {
		return stbi_write_bmp(path.c_str(), static_cast<int>(size_.x), static_cast<int>(size_.y), 4, pixels_.data()) !=
			   0;
	}
	if (extension == ".jpg" || extension == ".jpeg") {
		...
		return stbi_write_jpg(path.c_str(), static_cast<int>(size_.x), static_cast<int>(size_.y), 3, rgb.data(), 90) !=
			   0;
	}
...
Texture::Texture(const Texture& other) : size_(other.size_), pixels_(other.pixels_) {
	upload();
}
...
bool Texture::upload() {
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, static_cast<GLsizei>(size_.x), static_cast<GLsizei>(size_.y), 0, GL_RGBA,
