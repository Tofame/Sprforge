...
void EffectsScrollableWindow::selectEffect(int id, bool goToSelect) {
	...
	if (!Effects::isValidEffectTypeIndex(id)) {
		Warninger::sendWarning(FUNC_NAME,
							   "EffectType that we try to select doesn't exist (" + std::to_string(id) + ")");
		return;
	}
...
bool EffectsScrollableWindow::removeEffectType() {
	...
	if (selectedIndex >= getTotalButtons())
		selectedEffectIndex = getTotalButtons() - 1;
	return true;
}

void EffectsScrollableWindow::drawEffectTypeList(sf::Clock& deltaClock) {
	ImGui::BeginGroup();
	ImGui::Text("Effects list (Max effectType: %d)",
				(Effects::getEffectTypesCount() > 0 ? (Effects::getEffectTypesCount() - 1) : 0));
	...
	if (getSelectedButtonIndex() >= 0 && getSelectedButtonIndex() < (int)Effects::getEffectTypesCount()) {
		...
		if (ImGui::BeginTabBar("Tabs")) {
			if (ImGui::BeginTabItem("Texture")) {
				...
				ImVec2 centeredPos(
					groupSize.x / 2.0f -
						((static_cast<float>(spriteMaxSize) * static_cast<float>(unsavedEffectType->width)) / 2.0f),
					oldPos.y);
				centeredPos.x += spriteMaxSize / 4;
				ImVec2 gridPos = centeredPos;
				ImVec2 previewAreaSize(spriteMaxSize * unsavedEffectType->width,
									   spriteMaxSize * unsavedEffectType->height);
					...
					for (int w = 0; w < unsavedEffectType->width; w++) {
						for (int h = 0; h < unsavedEffectType->height; h++) {
							int spriteIndex = assetsManager->getTextureIdFromThingType(
								unsavedEffectType, w, h, assetsManager->getAnimationFrameSetting(), l, 0, 0, 0);
								...
								if (ImGui::BeginDragDropTarget()) {
									if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("TEXTURE_ID")) {
										...
										assetsManager->setTextureIdFromThingType(
											unsavedEffectType, w, h, assetsManager->getAnimationFrameSetting(),
				...
				ImGui::SliderInt("Animation Frame", &assetsManager->getAnimationFrameSettingRef(), 1,
								 unsavedEffectType->animationsFrames);
				...
				ImVec2 gridTotalSize =
					ImVec2(static_cast<float>(spriteMaxSize) * static_cast<float>(unsavedEffectType->width),
						   static_cast<float>(spriteMaxSize) * static_cast<float>(unsavedEffectType->height));
				if (drawGrid) {
		...
		ImGui::EndTabBar();
		// Save Effect Button - positioned at bottom center
		ImGui::Spacing();
		ImGui::Separator();
		ImGui::Spacing();
		...
		if (ImGui::Button("Save Effect", ImVec2(buttonWidth, 0))) {
