...
void BlitImage(sf::Image& destination, const sf::Image& source, unsigned destX, unsigned destY) {
	...
	for (unsigned y = 0; y < srcSize.y; ++y) {
		...
		for (unsigned x = 0; x < srcSize.x; ++x) {
			...
			if (outAlpha > 0.0f) {
				sf::Color blended;
				blended.r = static_cast<uint8_t>((srcColor.r * srcAlpha + dstColor.r * dstAlpha * (1.0f - srcAlpha)) /
												 outAlpha);
				blended.g = static_cast<uint8_t>((srcColor.g * srcAlpha + dstColor.g * dstAlpha * (1.0f - srcAlpha)) /
												 outAlpha);
				blended.b = static_cast<uint8_t>((srcColor.b * srcAlpha + dstColor.b * dstAlpha * (1.0f - srcAlpha)) /
												 outAlpha);
...
AssetsManager::AssetsManager(GUIHelper* guiHelper) {
	...
	sf::Image image({static_cast<unsigned>(spriteMaxSize), static_cast<unsigned>(spriteMaxSize)},
	...
	if (!BLANK_TEXTURE->loadFromImage(image)) {
		...
		if (!BLANK_TEXTURE->loadFromImage(fallbackImage)) {
			Warninger::sendErrorMsg(FUNC_NAME,
									"CRITICAL: Failed to create blank texture. Application may be unstable.");
		} else {
...
bool AssetsManager::loadSpr(const std::string& sprFilePath) {
	...
	for (uint32_t spriteId = 1; spriteId <= spriteCount; ++spriteId) {
		...
		if (offset == 0) {
			...
			if (!file.good()) {
				Warninger::sendWarning(FUNC_NAME, "Failed to seek to offset for sprite " + std::to_string(spriteId) +
													  ". Using blank texture.");
				texture = BLANK_TEXTURE;
			} else {
				...
				if (!file.good() || dataSize == 0) {
					...
					if (!file.good()) {
						Warninger::sendWarning(FUNC_NAME, "Failed to read sprite data for sprite " +
															  std::to_string(spriteId) + ". Using blank texture.");
						texture = BLANK_TEXTURE;
					} else {
						...
						while (pixelPtr < singleSpriteSize * singleSpriteSize && dataPtr < spriteData.size()) {
							...
							if (dataPtr + 2 > spriteData.size())
								break;
							uint16_t transparent = readLE16(&spriteData[dataPtr]);
							dataPtr += 2;
							pixelPtr += transparent;

							if (pixelPtr >= singleSpriteSize * singleSpriteSize)
								break;
							...
							for (uint16_t i = 0; i < colored; ++i) {
								if (pixelPtr >= singleSpriteSize * singleSpriteSize || dataPtr + 3 > spriteData.size())
									break;
								...
								uint8_t a = m_assetsInfo.transparency
												? (dataPtr < spriteData.size() ? spriteData[dataPtr++] : 255)
												: 255;
						...
						if (texture->loadFromImage(image)) {
							...
							Warninger::sendWarning(FUNC_NAME, "Failed to create texture for sprite " +
																  std::to_string(spriteId) + ". Using blank texture.");
							texture = BLANK_TEXTURE;
						}
					}
...
void AssetsManager::compileSprFromTextures(const std::string& fileName) {
	...
	for (uint32_t i = 0; i < spriteCount; ++i) {
		...
		auto isTransparent = [](const uint8_t* px, size_t i) {
			return (px[i * 4 + 0] == 255 && px[i * 4 + 1] == 0 && px[i * 4 + 2] == 255 && px[i * 4 + 3] == 255) ||
				   px[i * 4 + 3] == 0;
		};
	...
	try {
		...
		for (uint16_t id = 100; id < itemCount; ++id) {
			...
			if (itemType->hasLight()) {
				uint8_t flag = 0x15;
				outFile.write(reinterpret_cast<const char*>(&flag), sizeof(flag));
				outFile.write(reinterpret_cast<const char*>(&itemType->lightBlock.lightIntensity),
							  sizeof(itemType->lightBlock.lightIntensity));
				outFile.write(reinterpret_cast<const char*>(&itemType->lightBlock.lightColor),
							  sizeof(itemType->lightBlock.lightColor));
			}
			...
			uint32_t numSprites = static_cast<uint32_t>(itemType->width) * static_cast<uint32_t>(itemType->height) *
								  static_cast<uint32_t>(layers) * static_cast<uint32_t>(patternX) *
								  static_cast<uint32_t>(patternY) * static_cast<uint32_t>(patternZ) *
...
uint32_t AssetsManager::getTextureIdFromThingType(const std::shared_ptr<ThingType>& thingType, int w, int h, int a,
	...
	if (!thingType || thingType->width == 0 || thingType->height == 0 || thingType->layers == 0 ||
		thingType->patternX == 0 || thingType->patternY == 0 || thingType->patternZ == 0 ||
		thingType->animationsFrames == 0) {
		return 0;
	}

	if (w < 0 || w >= thingType->width || h < 0 || h >= thingType->height || layer < 0 || layer >= thingType->layers ||
		patternX < 0 || patternX >= thingType->patternX || patternY < 0 || patternY >= thingType->patternY ||
	...
	if (frame < 0)
		frame = 0;
	if (frame >= thingType->animationsFrames)
		frame = frame % thingType->animationsFrames;

	int spriteIndex =
		(((((((frame % thingType->animationsFrames) * thingType->patternZ + patternZ) * thingType->patternY +
			 patternY) *
				thingType->patternX +
			patternX) *
			   thingType->layers +
...
void AssetsManager::replacePreviewTexture(int thingTypeId, std::shared_ptr<sf::Texture> texture,
...
void AssetsManager::createPreviewTexture(int id, ThingCategory category) {
	...
	sf::Vector2u targetSize{static_cast<unsigned>(std::max(1.0f, buttonSize.x)),
...
void AssetsManager::createPreviewTexturesForPage(int pageFirstThingType, int pageLastThingType,
												 ThingCategory category) {
	for (int id = pageFirstThingType; id <= pageLastThingType; ++id) {
		...
		try {
			...
			Warninger::sendWarning(FUNC_NAME,
								   "Exception creating preview for thing " + std::to_string(id) + ": " + e.what());
			if (id >= previewTextures.size()) {
				previewTextures.resize(id + 1);
			}
			replacePreviewTexture(id, BLANK_TEXTURE, category);
...
sf::Texture AssetsManager::getPreviewImage(int thingTypeId, ThingCategory category, int animationFrame,
										   bool allLayers) {
	std::shared_ptr<ThingType> thingType;
	bool isValid = false;
	...
	case ThingCategory::ITEM:
		...
		if (isValid)
			thingType = Items::getItemType(thingTypeId);
		break;
	case ThingCategory::OUTFIT:
		isValid = Outfits::isValidOutfitTypeIndex(thingTypeId);
		if (isValid)
			thingType = Outfits::getOutfitType(thingTypeId);
		break;
	case ThingCategory::EFFECT:
		isValid = Effects::isValidEffectTypeIndex(thingTypeId);
		if (isValid)
			thingType = Effects::getEffectType(thingTypeId);
		break;
	case ThingCategory::MISSILE:
		isValid = Missiles::isValidMissileTypeIndex(thingTypeId);
		if (isValid)
			thingType = Missiles::getMissileType(thingTypeId);
		break;
	}
	...
	if (thingType->width == 0 || thingType->height == 0 || thingType->width > 255 || thingType->height > 255) {
		Warninger::sendWarning(FUNC_NAME, "Invalid dimensions for ThingType (" + std::to_string(thingTypeId) +
											  "): width=" + std::to_string(thingType->width) +
											  ", height=" + std::to_string(thingType->height));
		return sf::Texture(*BLANK_TEXTURE);
	}
	...
	sf::Vector2u size{static_cast<unsigned>(thingType->width * spriteMaxSize),
	...
	for (int l = startLayer; l <= endLayer; l++) {
		for (int w = 0; w < thingType->width; w++) {
			for (int h = 0; h < thingType->height; h++) {
				auto texture = getTexture(getTextureIdFromThingType(thingType, w, h, animationFrame, l, 0, 0, 0));
...
sf::Texture AssetsManager::getItemSpriteSheet(int itemTypeId, int animations) {
	...
	sf::Vector2u size{static_cast<unsigned>(itemType->width * spriteMaxSize),
			...
			for (int w = 0; w < itemType->width; w++) {
				for (int h = 0; h < itemType->height; h++) {
					...
					if (texture) {
						...
						unsigned destY =
							static_cast<unsigned>((itemType->height - h - 1) * spriteMaxSize + ((a - 1) * frameHeight));
	...
	if (ImGui::ImageButton("##ControlButton_CompileAssetsAs", compileAssetsAsIcon, {16, 16})) {
		if (m_assetsInfo.outputPath.empty()) {
			m_assetsInfo.outputPath = SavedData::getInstance()->getDataString("tempLoadedGraphicFilePath").empty()
										  ...
										  : Tools::cleanPathIntoFolderPath(
												SavedData::getInstance()->getDataString("tempLoadedGraphicFilePath"));
		}
...
void AssetsManager::onDatLoaded(const std::string& loadedPath) {
	...
	fmt::print("Total: {} items, {} outfits, {} effects, {} missiles loaded.\n", Items::getItemTypesCount(),
			   Outfits::getOutfitTypesCount(), Effects::getEffectTypesCount(), Missiles::getMissileTypesCount());
	...
	if (Missiles::getMissileTypesCount() > 0) {
		int firstMissile = 0;
		int lastMissile =
			std::min(firstMissile + buttonsPerPage, static_cast<int>(Missiles::getMissileTypesCount()) - 1);
...
void AssetsManager::loadThingTypePatterns(std::istream& inFile, std::shared_ptr<ThingType> thingType) {
	...
	// Validate dimensions
	if (thingType->width == 0)
		thingType->width = 1;
	if (thingType->height == 0)
		thingType->height = 1;
	...
	if (thingType->layers == 0)
		thingType->layers = 1;
	if (thingType->patternX == 0)
		thingType->patternX = 1;
	if (thingType->patternY == 0)
		thingType->patternY = 1;
	if (thingType->patternZ == 0)
		thingType->patternZ = 1;
	if (thingType->animationsFrames == 0)
		thingType->animationsFrames = 1;
	...
	uint32_t numSprites = static_cast<uint32_t>(thingType->width) * static_cast<uint32_t>(thingType->height) *
						  static_cast<uint32_t>(thingType->layers) * static_cast<uint32_t>(thingType->patternX) *
						  static_cast<uint32_t>(thingType->patternY) * static_cast<uint32_t>(thingType->patternZ) *
...
void AssetsManager::writeThingTypePatterns(std::ostream& outFile, std::shared_ptr<ThingType> thingType) {
	...
	uint32_t numSprites =
		thingType->width * thingType->height * layers * patternX * patternY * patternZ * animationsFrames;
...
void AssetsManager::doPopupAssetFileOpen() {
	...
	float bottomOffset = ImGui::GetContentRegionAvail().y - ImGui::CalcTextSize("SomeTextForSize").y -
	...
	ImGui::SameLine();
	float offset =
		ImGui::GetContentRegionAvail().x - ImGui::CalcTextSize("Cancel").x - ImGui::GetStyle().FramePadding.x * 2;
...
void AssetsManager::doPopupNewAssetFiles() {
	...
	if (currentVLabel.back() == '.')
		currentVLabel.pop_back();
	...
	std::string currentDimLabel = std::to_string(spriteDimensions[m_assetsInfo.dimensionIndex]) + "x" +
								  std::to_string(spriteDimensions[m_assetsInfo.dimensionIndex]);
