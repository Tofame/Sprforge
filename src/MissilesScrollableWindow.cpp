...
void MissilesScrollableWindow::selectMissile(int id, bool goToSelect) {
	...
	if (!Missiles::isValidMissileTypeIndex(id)) {
		Warninger::sendWarning(FUNC_NAME,
							   "MissileType that we try to select doesn't exist (" + std::to_string(id) + ")");
		return;
	}
...
bool MissilesScrollableWindow::removeMissileType() {
	...
	if (selectedIndex >= getTotalButtons())
		selectedMissileIndex = getTotalButtons() - 1;
	return true;
}

void MissilesScrollableWindow::drawMissileTypeList(sf::Clock& deltaClock) {
	ImGui::BeginGroup();
	ImGui::Text("Missiles list (Max missileType: %d)",
				(Missiles::getMissileTypesCount() > 0 ? (Missiles::getMissileTypesCount() - 1) : 0));
	...
	for (int i = startIndex; i < endIndex && i < (int)Missiles::getMissileTypesCount(); ++i) {
		...
		if (isSelected) {
			...
			ImGui::GetWindowDrawList()->AddRect(buttonPos,
...
void MissilesScrollableWindow::drawMissileTypePanel() {
	...
	if (getSelectedButtonIndex() >= 0 && getSelectedButtonIndex() < (int)Missiles::getMissileTypesCount()) {
		...
		if (ImGui::BeginTabBar("Tabs")) {
			if (ImGui::BeginTabItem("Texture")) {
				...
				if (unsavedMissileType->patternX >= 3 && unsavedMissileType->patternY >= 3) {
					...
					DirectionCell cells[3][3] = {{{"NW", 0, 0, false}, {"N", 1, 0, false}, {"NE", 2, 0, false}},
												 {{"W", 0, 1, false}, {"", -1, -1, true}, {"E", 2, 1, false}},
												 {{"SW", 0, 2, false}, {"S", 1, 2, false}, {"SE", 2, 2, false}}};

					float cellSize = static_cast<float>(spriteMaxSize) *
									 std::max(unsavedMissileType->width, unsavedMissileType->height);
					...
					if (ImGui::BeginTable("MissileDirectionsGrid", 3,
								...
								for (int l = 0; l < unsavedMissileType->layers; l++) {
									for (int w = 0; w < unsavedMissileType->width; w++) {
										for (int h = 0; h < unsavedMissileType->height; h++) {
											int spriteIndex = assetsManager->getTextureIdFromThingType(
												unsavedMissileType, w, h, assetsManager->getAnimationFrameSetting(), l,
												cell.patternX, cell.patternY, 0);
											auto texture = assetsManager->getTexture(spriteIndex);
											if (texture) {
												ImVec2 previewSize =
													ImVec2((float)texture->getSize().x, (float)texture->getSize().y);
												ImVec2 tempPos = cellMin;
												tempPos.x += std::floor((float)(unsavedMissileType->width - w - 1) *
																		spriteMaxSize);
												tempPos.y += std::floor((float)(unsavedMissileType->height - h - 1) *
																		spriteMaxSize);
												...
												if (ImGui::BeginDragDropTarget()) {
													if (const ImGuiPayload* payload =
															ImGui::AcceptDragDropPayload("TEXTURE_ID")) {
														int newTextureId = *(int*)payload->Data;
														assetsManager->setTextureIdFromThingType(
															unsavedMissileType, w, h,
															assetsManager->getAnimationFrameSetting(), newTextureId, l,
															cell.patternX, cell.patternY, 0);
												...
												if (w == unsavedMissileType->width - 1 &&
													h == unsavedMissileType->height - 1 && l == 0 && gridX == 0 &&
													gridY == 0) {
					...
					ImVec2 previewAreaSize(static_cast<float>(spriteMaxSize) * unsavedMissileType->width,
										   static_cast<float>(spriteMaxSize) * unsavedMissileType->height);
					...
					for (int l = 0; l < unsavedMissileType->layers; l++) {
						for (int w = 0; w < unsavedMissileType->width; w++) {
							for (int h = 0; h < unsavedMissileType->height; h++) {
								int spriteIndex = assetsManager->getTextureIdFromThingType(
									unsavedMissileType, w, h, assetsManager->getAnimationFrameSetting(), l, 0, 0, 0);
								auto texture = assetsManager->getTexture(spriteIndex);
								if (texture) {
									ImVec2 previewSize =
										ImVec2((float)texture->getSize().x, (float)texture->getSize().y);
									...
									tempPos.y +=
										std::floor((float)(unsavedMissileType->height - h - 1) * spriteMaxSize);
									...
									if (ImGui::BeginDragDropTarget()) {
										if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("TEXTURE_ID")) {
											int newTextureId = *(int*)payload->Data;
											assetsManager->setTextureIdFromThingType(
												unsavedMissileType, w, h, assetsManager->getAnimationFrameSetting(),
												newTextureId, l, 0, 0, 0);
											assetsManager->createPreviewTexture(getSelectedButtonIndex(),
																				ThingCategory::MISSILE);
										}
										ImGui::EndDragDropTarget();
									}
									if (w == unsavedMissileType->width - 1 && h == unsavedMissileType->height - 1 &&
										l == 0) {
										gridPos = ImGui::GetItemRectMin();
									}
								}
				...
				ImGui::SliderInt("Animation Frame", &assetsManager->getAnimationFrameSettingRef(), 1,
								 unsavedMissileType->animationsFrames);
				...
				ImVec2 gridTotalSize = ImVec2(static_cast<float>(spriteMaxSize) * unsavedMissileType->width,
											  static_cast<float>(spriteMaxSize) * unsavedMissileType->height);
				if (drawGrid) {
					for (int x = 0; x <= gridTotalSize.x; x += spriteMaxSize) {
						ImGui::GetWindowDrawList()->AddLine(ImVec2(gridPos.x + x, gridPos.y),
					...
					for (int y = 0; y <= gridTotalSize.y; y += spriteMaxSize) {
						ImGui::GetWindowDrawList()->AddLine(ImVec2(gridPos.x, gridPos.y + y),
				...
				if (ImGui::BeginTable("MissileTextureProperties", 3, ImGuiTableFlags_SizingFixedFit)) {
					...
					if (ImGui::InputInt("##Animations", &animations, 1, 1, ImGuiInputTextFlags_CharsDecimal)) {
						animations =
							std::clamp(animations, 1, ConfigManager::getInstance()->getItemMaxAnimationCount());
		...
		ImGui::EndTabBar();
		// Save Missile Button - positioned at bottom center
		ImGui::Spacing();
		ImGui::Separator();
		ImGui::Spacing();
		...
		if (ImGui::Button("Save Missile", ImVec2(buttonWidth, 0))) {
