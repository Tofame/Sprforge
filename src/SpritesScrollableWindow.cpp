...
void SpritesScrollableWindow::drawTextureList(sf::Clock& deltaClock) {
	...
	for (int i = startIndex; i < endIndex; ++i) {
		...
		ImGui::PushID(i);
		if (ImGui::ImageButton("##SpriteButton", (ImTextureID)(uintptr_t)texture->getNativeHandle(),
		...
		if (isSelected) {
			...
			ImGui::GetWindowDrawList()->AddRect(
				buttonPos, ImVec2(buttonPos.x + buttonSize.x, buttonPos.y + buttonSize.y), borderColor,
				0.0f, // Rounding
	...
	if (ImGui::BeginPopupModal("Wrong Texture Size", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
		...
		ImGui::Text(("Invalid image! You tried to import a sprite that is not size: " + std::to_string(spriteMaxSize) +
					 "x" + std::to_string(spriteMaxSize))
						.c_str());
		ImGui::Separator();
		if (ImGui::Button("OK")) {
			ImGui::CloseCurrentPopup();
	...
	bool hovered =
		(mouse_pos.x >= p_min.x && mouse_pos.x <= p_max.x && mouse_pos.y >= p_min.y && mouse_pos.y <= p_max.y);
	...
	if (hovered && !dropManager->GetDraggedFiles().empty()) {
		...
		if (!dropManager->IsDraggingFiles() && !dropManager->GetDraggedFiles().empty()) {
			...
			// TO-DO in the future when you also do the dragging for other things, you will probably need to clear it
			// elsewhere
...
void SpritesScrollableWindow::drawListControlButtons() {
	...
	if (ImGui::InputText("Sprite Id##SpriteIdSearchTextField", idInputBuffer, (int)sizeof(idInputBuffer),
	...
	if (ImGui::BeginPopupModal("Export Sprite Popup", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
		...
		ImGui::Text("Format:");
		ImGui::RadioButton("PNG", &formatSelected, 0);
		ImGui::SameLine();
		ImGui::RadioButton("BMP", &formatSelected, 1);
		ImGui::SameLine();
...
void SpritesScrollableWindow::exportTexture(Tools::EXPORT_OPTIONS option) {
	std::string filePath =
		(std::filesystem::path(outputFolder) / (Tools::trim(spriteName))).string() + getFormatString(option);
