...
ItemsScrollableWindow::ItemsScrollableWindow(AssetsManager* am) : ThingScrollableWindow(am, ThingCategory::ITEM) {
...
bool ItemsScrollableWindow::removeItemType() {
	...
	if (selectedIndex < 0 || selectedIndex > lastItemTypeId) {
		Warninger::sendWarning(FUNC_NAME, "Trying to remove ItemType, that does not exist (?) Id: " +
	...
	if (deletingLast) {
		...
		if (assetsManager->getUnsavedItemTypeId() != -1) {
			...
			Warninger::sendWarning(FUNC_NAME,
								   "No unsaved itemType detected, while removing: " + std::to_string(selectedIndex));
		}
	}
...
void ItemsScrollableWindow::drawItemTypeList(sf::Clock& deltaClock) {
	...
	for (int i = startIndex; i < endIndex; ++i) {
		...
		ImGui::PushID(i);
		if (ImGui::ImageButton("##ItemTypeButton", (ImTextureID)(uintptr_t)texture->getNativeHandle(),
...
void ItemsScrollableWindow::exportItem(Tools::EXPORT_OPTIONS option) {
	...
	std::string filePath =
		(std::filesystem::path(outputFolder) / (Tools::trim(itemName))).string() + Tools::getFormatString(option);
	...
	case Tools::BMP:
	case Tools::JPG:
		assetsManager->exportTexture(
			filePath, assetsManager->getItemSpriteSheet(getSelectedButtonIndex(), item->animationsFrames));
		break;
	case Tools::TOML:
...
void ItemsScrollableWindow::drawItemTypePanel() {
	...
	if (ImGui::BeginTabBar("Tabs")) {
		...
		if (getSelectedButtonIndex() >= 0 && getSelectedButtonIndex() < (int)Items::getItemTypesCount()) {
			if (assetsManager->getUnsavedItemTypeId() == -1 ||
				...
				assetsManager->setUnsavedItemType(Items::getItemType(getSelectedButtonIndex()),
		...
		if (ImGui::BeginTabItem("Texture")) {
			if (getSelectedButtonIndex() >= 0 && getSelectedButtonIndex() < (int)Items::getItemTypesCount()) {
							...
							for (int w = 0; w < previewIt->width; w++) {
								for (int h = 0; h < previewIt->height; h++) {
									int spriteIndex = assetsManager->getTextureIdFromItemType(
										previewIt, w, h, assetsManager->getAnimationFrameSetting(), layerIdx, pX, pY,
										pZ);
									...
									if (texture) {
										...
										if (pX == 0 && pY == 0 && pZ == 0 && w == previewIt->width - 1 &&
											h == previewIt->height - 1) {
											gridPos = ImGui::GetItemRectMin();
										}
									} else {
										Warninger::sendWarning(FUNC_NAME,
															   "No texture detected while displaying item's textures");
									}
								}
							}
						}
				...
				ImVec2 gridTotalSize =
					ImVec2(static_cast<float>(totalPreviewWidth), static_cast<float>(totalPreviewHeight));
				if (drawGrid) {
		...
		if (ImGui::BeginTabItem("Properties")) {
			...
			if (ImGui::CollapsingHeader("Minimap Color", ImGuiTreeNodeFlags_DefaultOpen)) {
				...
				// The trigger button
				if (ImGui::ColorButton("##CurrentMinimapColor", currentColor, ImGuiColorEditFlags_NoTooltip,
									   ImVec2(30, 30))) {
			...
			if (ImGui::CollapsingHeader("Flags", ImGuiTreeNodeFlags_DefaultOpen)) {
				...
				if (ImGui::BeginTable("BooleanItemProperties", 2)) {
					...
					const std::vector<std::pair<const char*, ItemTypeFlags>> flagNames = {
						{"Ground", IS_GROUND},		{"Container", IS_CONTAINER}, {"Stackable", STACKABLE},
						{"Unpassable", UNPASSABLE}, {"Unmovable", UNMOVABLE},	 {"Force Use", FORCE_USE},
						{"MultiUse", MULTI_USE},	{"Pickupable", PICKUPABLE},	 {"Block Missile", BLOCK_MISSILE}};
		...
		if (getSelectedButtonIndex() >= 0 && getSelectedButtonIndex() < (int)Items::getItemTypesCount()) {
			if (assetsManager->getUnsavedItemTypeId() != -1 &&
				...
				Warninger::sendWarning(
					FUNC_NAME,
					"Current UnsavedItemTypeid is not the same as selection. This warning shouldn't happen.");
			}
		}
	...
	if (ImGui::BeginPopupModal("Unsaved Item Changes", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
		std::string text =
			std::string("Save Changes in the Item ") + std::to_string(assetsManager->getUnsavedItemTypeId()) + "?";
