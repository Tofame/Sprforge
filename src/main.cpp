...
int main() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		sf::Vector2u windowSize{static_cast<unsigned>(std::max(frameBufferWidth, 0)),
		...
		displayExitConfirmation(window, appContext.showExitConfirmation, assetsManager->isCompilable(), assetsManager);
...
void pasteFromClipboard(AssetsManager* am, SpritesScrollableWindow* spritesWindow, ItemsScrollableWindow* itemsWindow) {
	...
	if (selectedCategory == CATEGORY_SPRITES && hasBitmap) {
		...
		if (Tools::pasteTextureFromClipboard(pastedTexture)) {
			...
			if (pastedTexture->getSize().x != spriteMaxSize || pastedTexture->getSize().y != spriteMaxSize) {
				Warninger::sendWarning(FUNC_NAME, "Clipboard Texture: texture size must be " +
													  std::to_string(spriteMaxSize) + "x" +
													  std::to_string(spriteMaxSize));
				return;
			}
