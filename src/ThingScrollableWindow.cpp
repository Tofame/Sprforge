...
void ThingScrollableWindow::drawPaginationControls() {
	...
	drawPaginationRow(startIndex, endIndex, typeNameCapitalized.c_str(),
					  (std::string(typeNameCapitalized) + " Id").c_str());

	ImGui::Spacing();

	// Action buttons - second row
	drawActionButtonsRow((std::string("New ") + typeNameCapitalized).c_str(),
...
void ThingScrollableWindow::drawPaginationRow(int startIndex, int endIndex, const char* typeName,
	...
	if (ImGui::InputText(inputId.c_str(), idInputBuffer, sizeof(idInputBuffer),
...
std::string ThingScrollableWindow::getInputTextFieldId() const {
	return std::string(ThingCategoryToString(category)) + "TypeIdSearchTextField##" +
		   std::string(ThingCategoryToString(category)) + "Id";
}
...
void ThingScrollableWindow::drawLightControlSegment(ThingType& thing) {
	...
	if (open) {
		...
		// The trigger button
		if (ImGui::ColorButton("##CurrentLightControlColor", currentColor, ImGuiColorEditFlags_NoTooltip,
							   ImVec2(30, 30))) {
		...
		if (ImGui::BeginPopup("LightControlColorPicker")) {
			...
			for (int idx = 0; idx < totalColors; ++idx) {
				...
				if (ImGui::ColorButton("##pick", c, ImGuiColorEditFlags_NoTooltip | ImGuiColorEditFlags_NoBorder,
