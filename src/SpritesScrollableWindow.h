...
public:
	...
	int getTotalButtons() { return (int)assetsManager->getTextureCount(); }
	int getSelectedSpriteIndex() { return selectedButtonIndex; }
	...
	int getPageFirstIndex() { return currentPage * ConfigManager::getInstance()->getButtonsCountSpritePage(); }
	int getPageLastIndex() {
		return std::min(getPageFirstIndex() + ConfigManager::getInstance()->getButtonsCountSpritePage(),
						getTotalButtons());
	}
	void incrementPage() {
		if (!onPageChange()) {
	...
	void setDropManager(DropManager* _dm) { this->dropManager = _dm; }

	DropManager* getDropManager() const { return dropManager; }
